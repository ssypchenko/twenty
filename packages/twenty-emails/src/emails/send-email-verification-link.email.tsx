...
export const SendEmailVerificationLinkEmail = ({
  ...
  const bodyId = isEmailUpdate
    ...
    : 'Thanks for creating an account. Click below to verify your email address.';
  const ctaLabel = isEmailUpdate
    ? i18n._('Confirm new email')
    : i18n._('Verify Email');

  return (
    <BaseEmail width={333} locale={locale} logoBaseUrl={link}>
      <Title value={title} />
      <MainText>
        <Trans id={bodyId} />
      </MainText>
...
SendEmailVerificationLinkEmail.PreviewProps = {
  link: 'https://crm.example.com/verify-email/123',
  locale: 'en',
  isEmailUpdate: false,
};
