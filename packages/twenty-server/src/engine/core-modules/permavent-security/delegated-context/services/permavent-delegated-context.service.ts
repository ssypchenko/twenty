...
} from 'src/engine/core-modules/permavent-security/delegated-context/constants/permavent-delegated-context-header.constant';
...
export class PermaventDelegatedContextService {
  constructor(
    ...
    private readonly delegatedAuditService: PermaventDelegatedAuditService,
    @InjectRepository(UserWorkspaceEntity)
    private readonly userWorkspaceRepository: Repository<UserWorkspaceEntity>,
  ) {}

  public async resolveForRequest({
    authContext,
    headers,
  }: {
    ...
    if (!authContext.apiKey || !authContext.workspace) {
      this.recordResolutionDenied(authContext, 'non_api_key_actor_header');
    ...
    if (
      ...
      this.recordResolutionDenied(authContext, 'feature_disabled');

      throw this.forbidden('Delegated API context is disabled');
    }

    if (!this.getTrustedApiKeyIds().has(authContext.apiKey.id)) {
      this.recordResolutionDenied(authContext, 'api_key_not_allowlisted');
    ...
    if (!emailSchema.safeParse(actorEmail).success) {
      this.recordResolutionDenied(authContext, 'malformed_actor_email');
    ...
    try {
      ...
      this.recordResolutionDenied(authContext, 'actor_unavailable');

      throw error;
    }

    return { ...authContext, delegatedActor };
  }
  ...
  private assertTrustedApiKeyHasActorHeader(authContext: RawAuthContext): void {
    ...
    this.recordResolutionDenied(authContext, 'missing_actor_header');
  ...
  private recordResolutionDenied(
    authContext: RawAuthContext,
    denialCategory: string,
  ): void {
    this.delegatedAuditService.recordResolutionDenied({
