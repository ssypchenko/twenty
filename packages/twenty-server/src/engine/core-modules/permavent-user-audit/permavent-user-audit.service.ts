...
export class PermaventUserAuditService {
  ...
  async recordRlsDenied(input: {
    ...
    if (
      ...
      return;
    try {
      const actorDisplayNamesByWorkspaceMemberId =
        await this.getActorDisplayNamesByWorkspaceMemberId(input.workspaceId, [
          input.workspaceMemberId,
        ]);
      ...
      await this.repository.insert({
        ...
        actorDisplayName:
          actorDisplayNamesByWorkspaceMemberId.get(input.workspaceMemberId) ??
          null,
        action: 'RLS_DENIED',
        result: 'DENIED',
